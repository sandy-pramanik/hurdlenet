...
model{
  ...
  for(t in 1:nTime){
    ...
    target += normal_lpdf(logZ11[t]/exp(h[(t-1)*n + 1]/2) | 0, 1) -
              h[(t-1)*n + 1]/2;
    target += normal_lpdf(Zrestv[((t-1)*(n-1 + n*(K-1) - (K*(K-1))/2) + 1):((t-1)*(n-1 + n*(K-1) - (K*(K-1))/2) + n-1)] ./ exp(h[((t-1)*n + 2):(t*n)]/2) | 0, 1) -
              h[((t-1)*n + 2):(t*n)]/2;
    
    for(k in 2:K){
      
      target += normal_lpdf(Zrestv[((t-1)*(n-1 + n*(K-1) - (K*(K-1))/2) + n-1 + n*(k-2) - ((k-1)*(k-2))/2 + 1):((t-1)*(n-1 + n*(K-1) - (K*(K-1))/2) + n-1 + n*(k-1) - (k*(k-1))/2)] ./ exp(h[((t-1)*n + k):(t*n)]/2) | 0, 1) -
                h[((t-1)*n + k):(t*n)]/2;
